...
if __name__ == '__main__':
    ...
    domain_ranges = np.array( [ (min(data_xs[:,d]),max(data_xs[:,d])) for d in range(n_features) ] )
    
    cmap = cm.get_cmap('inferno_r')

    fig,(ax1,ax2,ax3) = plt.subplots( 1,3,figsize = (9.,3.) )
    for label,cval in zip([0,1],[0.2,0.8]):
        ...
        if label == 0:
            ...
            class_label = 'Outlier'
        ax1.scatter(data_xs[idxs,0],data_xs[idxs,1],color=color,s=5,alpha=0.8,label=class_label)
